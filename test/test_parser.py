...
def test_embedded_parsing():
    ...
    assert result["items"][0]["properties"]["content"][0]["html"] == '<p>Blah blah blah blah blah.</p>\n   <p>Blah.</p>\n   <p>Blah blah blah.</p>'
    assert result["items"][0]["properties"]["content"][0]["value"] == 'Blah blah blah blah blah.\nBlah.\nBlah blah blah.'
...
def test_template_tag_inside_e_value():
    ...
    assert "This is a Test with a <code>template</code> tag after this:" == result['items'][0]['properties']['content'][0]['html']
    assert "This is a Test with a template tag after this:" == result['items'][0]['properties']['content'][0]['value']
...
def test_area_uparsing():
    ...
    assert {'url': ['http://suda.co.uk'], 'name': ['Brian Suda']} == result["items"][0]["properties"]
...
def test_enclosures():
    ...
    assert {'http://example.com/movie.mp4': {
        'rels': ['enclosure'],
        'text': 'my movie',
        'type': 'video/mpeg'}
    } == result['rel-urls']
...
def test_relative_datetime():
    ...
    assert result[u'items'][0][u'properties'][u'updated'][0] == '2015-01-02 05:06'

def test_stop_implied_name_nested_h():
    result = parse_fixture("implied_properties/stop_implied_name_nested_h.html")
    assert 'name' not in result[u'items'][0][u'properties']
...
def test_implied_properties_silo_pub():
    ...
    assert 'name' not in item[u'properties']
...
def test_value_name_whitespace():
    ...
    for i in range(8, 10):
        assert result["items"][i]["properties"]["content"][0]["value"] == "One\nTwo\nThree"
        assert result["items"][i]["properties"]["name"][0] == "One\nTwo\nThree"

    assert result["items"][10]["properties"]["content"][0]["value"] == "Hello World      one\n      two\n      three\n    "
    assert result["items"][10]["properties"]["name"][0] == "Hello World      one\n      two\n      three\n    "

    assert result["items"][11]["properties"]["content"][0]["value"] == "Correct name Correct summary"
...
def test_backcompat_hentry():
    ...
    assert 'Tom Morris' == result['items'][0]['properties']['author'][0]['properties']['name'][0]
...
def test_backcompat_hproduct():
    ...
    assert "Magical tasty sugar pills that don't do anything." == result["items"][0]["properties"]['description'][0]
...
def test_backcompat_rel_bookmark():
    ...
    for result in results:
        assert ['https://example.com/bookmark', 'https://example.com/bookmark-url'] == result['items'][0]['properties']['url']
...
def test_backcompat_rel_tag():
    ...
    results = [parse_fixture(x) for x in tests]
    for result in results:
        assert ['cat', 'dog', 'mountain lion', 'mouse', 'meerkat'] == result['items'][0]['properties']['category']
...
def test_backcompat_nested_mf2_in_mf1():
    ...
    assert 'Correct summary' == result['items'][0]['children'][0]['properties']['summary'][0]
...
def test_photo_with_alt():
    ...
    assert '/photo.jpg' == result['items'][3]['properties']['in-reply-to'][0]['properties']['photo'][0]
    ...
    assert '/photo.jpg' == exp_result['items'][3]['properties']['in-reply-to'][0]['properties']['photo'][0]
    assert '/photo.jpg' == exp_result['items'][3]['properties']['in-reply-to'][0]['value']
    ...
    assert '/photo.jpg' == exp_result['items'][4]['properties']['in-reply-to'][0]['value']
    assert 'alt text' == exp_result['items'][4]['properties']['in-reply-to'][0]['properties']['photo'][0]['alt']
    ...
    assert '/photo.jpg' == exp_result['items'][5]['properties']['in-reply-to'][0]['value']
    assert '' == exp_result['items'][5]['properties']['in-reply-to'][0]['properties']['photo'][0]['alt']
...
def test_all_u_cases():
    ...
    for i in range(URL_COUNT):
        make_labelled_cmp("all_u_cases_" + str(i))("http://example.com/test", result['items'][0]['properties']['url'][i])
